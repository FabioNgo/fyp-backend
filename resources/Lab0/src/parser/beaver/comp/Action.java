...
package parser.beaver.comp;

import parser.beaver.comp.util.BitSet;
import parser.beaver.comp.util.Log;
import parser.beaver.spec.*;
...
class Action
{
	...
	static class Shift extends Action
	{
		...
		{
			super(Action.Type.SHIFT, lookahead);
			this.state = state;
		}
		...
		{
			if (!(act instanceof Action.Reduce))
				throw new IllegalArgumentException("shift-reduce expected, \"" + act + "\" found");

			Action.Reduce reduce_act = (Action.Reduce) act;
			Terminal reduce_prec_sym = reduce_act.rule.prec_sym;

			if (this.lookahead instanceof NonTerminal)
			{
				act.type = new Action.Type.Conflict.ShiftReduce(this, reduce_act, act_state, lookahead.name +  " is a non-terminal");
				return false;
			}
			...
			if (shift_prec_sym.prec < reduce_prec_sym.prec)
			{
				...
				this.type = Action.Type.RESOLVED;
				return true;
			}
			//
			...
			act.type = new Action.Type.Conflict.ShiftReduce(this, reduce_act, act_state, shift_prec_sym.prec > 0 ? lookahead.name +  " is nonassociative" : "insufficient precedence information");
			return false;
		}
	...
	static class Reduce extends Action
	{
		...
		static class Maker extends BitSet.Processor
		{
			...
			{
				state.actions.add(new Action.Reduce(terminals[index], rule));
			}
		}
		...
		{
			super(Action.Type.REDUCE, lookahead);
			this.rule = rule;
		}
		...
		boolean resolveConflict(Action act, State act_state, Log log)
		{
			...
			act.type = new Action.Type.Conflict.ReduceReduce(this, reduce_act, act_state, "equal precedence");
			return false;
		}
	...
	static class Accept extends Action
	{
		...
		{
			super(Action.Type.ACCEPT, grammar.goal_symbol);
