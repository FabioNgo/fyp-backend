...
package parser.beaver.spec;

import parser.beaver.spec.Production.RHS;
import parser.beaver.Symbol;
import parser.beaver.comp.util.Log;
import parser.beaver.spec.ast.Declaration;
import parser.beaver.spec.ast.GrammarTreeRoot;
import parser.beaver.spec.ast.Rule;
import parser.beaver.spec.ast.TreeWalker;
...
public class GrammarBuilder extends TreeWalker
{
	...
	public void visit(GrammarTreeRoot root)
	{
		...
		else // parser needs a synthetic goal
		{
		    ...
		    rules.add(new Production(rules.size(), grammar.goal_symbol, new RHS(alts[0])));
			...
			n_terms++;
			rules.add(new Production(rules.size(), grammar.goal_symbol, new RHS(term, alts[i])));
		    }
		}
		...
		root.accept(new RuleWalker() /* grammar's goal cannot be used in any RHS */
		{
			...
			public void visit(GrammarTreeRoot root)
			{
				...
				if (found)
				{
					...
					rules.add(new Production(rules.size(), new_goal_sym, new RHS(grammar.goal_symbol)));
		...
		root.accept(new RuleWalker() /* build production rules */
		{
			...
			public void visit(Rule.Definition rhs)
			{
				...
				Production rule =
					new Production( rules.size()
				                       , lhs_sym
						       , new RHS (
								  (RHS.Item[]) rhs_elements.toArray(new RHS.Item[rhs_elements.size()]))
			...
			public void visit(Rule.Definition.Element rhs_item)
			{
				...
				rhs_elements.add(new RHS.Item(rhs_sym, rhs_item.getAlias()));
			}
			...
			private NonTerminal getOpt(String sym_name)
			{
				...
				if (opt_sym == null)
				{
					...
					n_nonterms++;
					rules.add(new Production(rules.size(), opt_sym, new RHS()));
					rules.add(new Production(rules.size(), opt_sym, new RHS(item_sym)));
				}
				return opt_sym;
			}
			...
			private NonTerminal getLst(String sym_name)
			{
				...
				if (lst_sym == null)
				{
					...
					rules.add(new Production(rules.size(), lst_sym, new RHS(item_sym)));
					rules.add(new Production(rules.size(), lst_sym, new RHS(lst_sym, item_sym)));
				}
				return lst_sym;
			}
		});
        ...
        if(className.endsWith("Y")){
          className = className.substring(0,className.length()-1);
			...
			new_list_rule.code = new StringBuffer(Grammar.EBNF_LIST_TYPE_NAME.length() * 2 + 77)
				.append(String.format("if(_symbols[offset +1 ] instanceof %s){\n" +
          "            return _symbols[offset + 1];  \n" +
          "          }else{\n" +
          "            %s<%s> lst = new %s();\n" +
          "            return lst.add((%s)_symbols[offset + 1]);\n" +
          "          }", Grammar.EBNF_LIST_TYPE_NAME,Grammar.EBNF_LIST_TYPE_NAME,className,Grammar.EBNF_LIST_TYPE_NAME,className))
				.toString();

			add_elem_rule.code = new StringBuffer(Grammar.EBNF_LIST_TYPE_NAME.length() + 88)
			.append("((").append(Grammar.EBNF_LIST_TYPE_NAME).append(") _symbols[offset + 1].value).add((").append(className).append(")_symbols[offset + ").append(add_elem_rule.rhs.size()).append("]").append(elem.type != null ? ".value" : "").append("); ")
